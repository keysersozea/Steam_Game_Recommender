...
def create_similarUser_df(similarity_matrix, user, threshold=0.01):
    ...
    # 'threshold' değerinden yüksek 'similarity' değerlerine sahip satırları filtrele
    filtered_rows = []
    for index, row in similar_user_1.iterrows():
        if row['similarity'] > threshold:
            filtered_rows.append(row)

    # Filtrelenmiş satırları DataFrame'e dönüştür
    similar_user_filtered = pd.DataFrame(filtered_rows)
...
def user_based_suggestions(user, df):
    ...
    log_transform(df)
    weight_with_monthsago(df, weighting_factor=0.03)
    ...
    similar_user_df = create_similarUser_df(similarity_matrix, user, threshold=0.2)
    ...
    recommendation_df = scale_w_similarity(recommendation_df, exponent=0.3)
    ...
    user_based_rec_df = finalize_recommendation_df(recommendation_df, played_games, num_games=30)
    return user_based_rec_df, general_df, played_games_df, products
